...
const ReservationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Vérifier que la réponse est bien en format JSON
      if (response.ok) {
        const data = await response.json();
        alert(data.message); // Afficher le message reçu
      } else {
        // Si la réponse n'est pas OK, afficher un message d'erreur
        const errorData = await response.text();
        alert('Erreur : ' + errorData); // Afficher l'erreur retournée par le serveur
      }
    } catch (error) {
      // Gestion des erreurs de réseau ou autres erreurs liées à la soumission
